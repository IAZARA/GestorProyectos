...
const apiPath = path.join(process.cwd(), 'db-users-api.js');
...
const backupPath = `${userStorePath}.db-backup`;
...
const API_URL = 'http://137.184.198.221:3005/api/users';
...
try {
  ...
  execSync('pm2 start db-users-api.js --name users-api');
  execSync('pm2 save');
  console.log('API configurada correctamente en PM2.');
} catch (error) {
  console.error('Error al configurar PM2:', error.message);
  console.log('Por favor, configura manualmente: pm2 start db-users-api.js --name users-api');
}
